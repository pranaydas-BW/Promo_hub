...
export default function TodayPromos() {
  ...
  const matchDate = (row, field, date) =>
    Array.isArray(row.date_ranges) && row.date_ranges.some(dr => dr[field] === date)

  const matchStore = (row) => store === 'All' || (row.store || '').includes(store)
...
function PromoGroup({ title, icon, color, bg, border, rows, event, matchDate, onExport, onPick }) {
  return (
    <div className="bg-white border border-border rounded-xl overflow-hidden">
      ...
      {rows.length === 0 ? (
        <div className="py-10 text-center"><p className="text-sm font-body text-muted opacity-60">Nothing here.</p></div>
      ) : (
        <div className="divide-y divide-border">
          {rows.map(r => <PromoCard key={r.id} row={r} event={event} matchDate={matchDate} color={color} onPick={onPick} />)}
        </div>
      )}
    </div>
  )
...
function PromoCard({ row: r, event, matchDate, color, onPick }) {
    ...
    <div className={\`p-4 \${isNewPromo && !isPicked ? 'border-l-4 border-l-accent bg-orange-50/30' : isPicked ? 'bg-gray-50 opacity-70' : ''}\`}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {isNewPromo && !isPicked && <span className="text-[10px] font-mono font-bold bg-accent text-white px-1.5 py-0.5 rounded-full">NEW</span>}
      ...
      <div className="flex flex-wrap items-center gap-3 mt-2 text-[11px] text-muted">
        ...
        {r.sku_file_link && <a href={r.sku_file_link} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-accent hover:underline"><ExternalLink size={9} /> SKU File</a>}
      </div>
    </div>
  )
}
